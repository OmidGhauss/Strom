...
export function buildOfferEmailHtml(
  ...
  const rows: string[] = [
    row("Angebotsnummer", `${escapeHtml(offer.offer_number)} (Version ${escapeHtml(String(offer.version))})`, false),
...
<body style="margin:0;padding:0;font-family:Arial,Helvetica,sans-serif;background:#f5f5f5">
  <div style="max-width:600px;margin:0 auto;background:#ffffff">
    ...
    <div style="padding:32px">
      ...
      ${messageHtml}
      <p style="margin:0 0 24px">anbei finden Sie Ihr persönliches ${escapeHtml(energyLabel)}angebot:</p>
      <table style="width:100%;border-collapse:collapse;margin-bottom:24px">
        ${rows.join("\n        ")}
      </table>
      <p style="margin:0 0 8px">Das vollständige Angebot finden Sie im Anhang als PDF-Datei.</p>
